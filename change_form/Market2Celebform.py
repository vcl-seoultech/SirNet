...
def main(market_path: str, save_path: str):
    """
    :param market_path:
    :param save_path:
    :return True:
    ...
    # train
    train_imgs = pt.sorted_glob(f'{market_path}/bounding_box_train/*.jpg')
    ...
    gallery_id = sorted(glob.glob(f'{market_path}/bounding_box_test/*.jpg'))
    for n, img in pt.ProgressBar(enumerate(gallery_id), finish_mark='finished gallery set in validation data'):
        ...
        if fore_id == 0 or fore_id == -1: continue
        shutil.copy(img, f'{save_path}/gallery/{fore_id}_{cam}_{n + 1}.jpg')

    # query
    print('start query set')
    query_id = sorted(glob.glob(f'{market_path}/query/*.jpg'))
    _id = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./market_celeb_form')
    args = parser.parse_args()

    main(args.market_root, args.save_path)
